...
//        $sections = ClassRoutine::pluck('section_id')->unique();
	...
	public static function getRoutine($class="",$section=""){
		...
		$subjects = DB::select("SELECT routine.*,teachers.name as teacher FROM (SELECT subject.*, class_routines.*
FROM (SELECT class_days.id as class_day_id,class_days.day as class_day, subjects.id as s_id,subjects.subject_name
		FROM class_days JOIN subjects WHERE class_days.is_active=1 AND subjects.class_id='$class')
    as subject LEFT JOIN class_routines ON subject.s_id=class_routines.subject_id AND subject.class_day=class_routines.day AND
		class_routines.section_id='$section') as routine,assign_subjects,teachers
WHERE assign_subjects.section_id='$section' AND routine.s_id=assign_subjects.subject_id
  AND assign_subjects.teacher_id=teachers.id
