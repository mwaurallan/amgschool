...
//        join('users','users.id','=','students.user_id')
	...
	public function feeStatement(Request $request){
        $students = Student::where('parent_id',get_parent_id())
            ->get();
        if(count($students) == 1){
            $statements = StudentAccount::where('student_id',$students->first()->id)
                ->get();

            $stu = Student::find($students->first()->id);
            return view('backend.private.parent.my-student-statement',[
                'students' => $students,
