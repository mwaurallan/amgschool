...
class ReportController extends Controller
{
	...
	public function studentStatement(Request $request){
        if(!$request->isMethod('POST')){
            return view('backend.reports.student-statement',[
                'students' => Student::all()
            ]);
        }

        $statements = StudentAccount::where('student_id',$request->student_id)
            ->with(['payment'])->orderBy('date')
            ->get();
...
//			AND marks.class_id=:class
//GROUP BY marks.exam_id", ["student_id" => $student_id, "class" => $class_id]
//            );
			$exams = DB::select("SELECT marks.exam_id,marks.class_id,marks.section_id,marks.subject_id, exams.name
			FROM marks,exams WHERE marks.exam_id=exams.id AND marks.student_id=:student_id
			AND marks.class_id=:class "
                , ["student_id" => $student_id, "class" => $class_id]
            );
	...
	public function studentAccounts(Request $request){
        ...
        if(!$request->isMethod('POST')){
            return view('backend.reports.student-accounts',[
                'classes' => $classes
            ]);
        }
        ...
        $currentClass= '';
        $students = Student::when($request->class != 'all',function($q)use($request,$currentClass){

            $class = StudentSession::where('class_id',$request->class)
                ->where('session_id',StudentSession::max('session_id'))->pluck('student_id')->toArray();
//            $currentClass = $class;
            return $q->whereIn('id',$class);
        })
//            ->orderBy('register_no')
        ->get();
        $reports = [];
        if(count($students)){
            foreach ($students as $student){
                $bbfCredits = StudentAccount::where('student_id',$student->id)
                    ->where('transaction_type','CREDIT')
                    ->where('date','<',$from)->sum('amount');
                $bbfDebits = StudentAccount::where('student_id',$student->id)
                    ->where('transaction_type','DEBIT')
                    ->where('date','<',$from)->sum('amount');
                $currentCredits = StudentAccount::where('student_id',$student->id)
                    ->where('transaction_type','CREDIT')
                    ->whereBetween('date',[$from,$to])->sum('amount');
                $currentDebits = StudentAccount::where('student_id',$student->id)
                    ->where('transaction_type','DEBIT')
                    ->whereBetween('date',[$from,$to])->sum('amount');
                $bcf = StudentAccount::where('student_id',$student->id)
                    ->where('transaction_type','CREDIT')->sum('amount') - StudentAccount::where('student_id',$student->id)
                        ->where('transaction_type','DEBIT')->sum('amount');
                $cc = ClassModel::find(StudentSession::where('student_id',$student->id)->orderByDesc('session_id')->first()->class_id)->class_name ?? '';

                $reports[] = [
                    'studentId' => $student->id,
                    'name' => $student->first_name.' '.$student->last_name,
                    'bbf' => $bbf = $bbfCredits - $bbfDebits,
                    'currentFees' => $currentCredits ,
                    'totalBal' => $currentCredits + $bbf ,
                    'currentPayments' => $currentDebits,
                    'arrears' => $bcf,
                    'class' => $cc,
                    'admn' => $student->register_no
                ];
        ...
        return view('backend.reports.student-accounts',[
            'classes' => $classes,
            'reports' => collect($reports)->sortBy('admn'),
            'date1' => $from->toDateString(),
            'date2' => $to->toDateString()
        ]);
    }
	public function studentBills(Request $request){
        $classes = ClassModel::all();
        if(!$request->isMethod('POST')){
            return view('backend.reports.student-bills',[
                'classes' => $classes
            ]);
        }
        $from = Carbon::parse($request->from)->startOfDay();
        ...
        $students = Student::when($request->class != 'all',function($q)use($request){
...
//            ->orderBy('register_no')
        ...
        if(count($students)){
            foreach ($students as $student){
                ...
                $bills = Invoice::whereBetween('date',[$from,$to])
                    ->where('student_id',$student->id)
                    ->pluck('id');
                $billDetails = InvoiceItem::whereIn('invoice_id',$bills->toArray())
//                    ->with(['fee'])
                    ->get();
                $individualBs = $billDetails->pluck('fee_id')->unique();
//                print_r($individualBs);die;
                $detail = [];
                if(count($individualBs)){
                    foreach ($individualBs as $bill){
                        $detail[] = [
                            'feeType' => $name = FeeType::find($bill)->fee_type ?? '',
                            'fee_type_id' => $bill,
                            'amount' => $billDetails->where('fee_id',$bill)->sum('amount')
                        ];
                        if(!$individualBills->firstWhere('fee_type_id',$bill)){
